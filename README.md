...
- **Transit** — public transit route network with visibility boost on dark maps
- **Crash Data** — Texas ZIP-level incident markers, pulsing and color-scaled by intensity, with statistics popup on click
- **Weather** — combined NOAA severe weather alert polygons + Google Weather current conditions widget
...
- **Earthquakes** — last 30 days of seismic activity in Texas (USGS, magnitude 1.0+)

### Location Intelligence
- **Street View** — windowed panorama viewer with title bar, address label, expand and close controls
...
### Map Interaction
- **Right-click context menu** — Street View, Measure Distance, Print — anchored to cursor position
- **Measure tool** — fixed-pixel dots and dashed line, live waypoint panel showing start, intermediate points, end and running distance in miles and kilometers
...
- **Global API call limit** — 50 calls per session across all external APIs, auto-disables on limit
